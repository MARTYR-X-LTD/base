// cms/src/instrumentation.node.ts
// Node.js-only boot logic. Only imported when NEXT_RUNTIME === 'nodejs'.
// Top-level Node.js imports are safe here — this file never enters the edge bundle.
...
/**
 * The actual cleanup logic, exported so tests can await it directly.
 */
export async function runBootCleanup(payload: Payload): Promise<void> {
  ...
  for (const docId of tempDocIds) {
    try {
      ...
      if (status === 404) {
        ...
        const orphanedKeys = [...imageKeys, ...videoKeys]

        if (orphanedKeys.length > 0) {
          payload.logger.info(
            `[Boot] Deleting ${orphanedKeys.length} orphaned R2 files for ${docId}`,
          )
  ...
  const avifencFiles = tmpFiles.filter(
    (f) => (/\.avif$/.test(f) || /\.png$/.test(f)) && /^[0-9a-f]{8}-[0-9a-f]{4}-/.test(f),
  )
...
/**
 * Fire-and-forget wrapper called by Payload's onInit hook.
 */
export function bootCleanup(payload: Payload): void {
  payload.logger.info('[Boot] Running boot-time cleanup...')
  runBootCleanup(payload).catch((err) => {
